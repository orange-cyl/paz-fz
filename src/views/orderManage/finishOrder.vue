...
<template>
  <div id="waitOrder">
    <div id="orderManage">
    <div class="totalTitle">已完成订单</div>
    <div class="orderManage-option">
     
      <div class="searchOption">
        <div>
          <el-date-picker style="width:190px;" size='mini' v-model="searchObj.searchTime" type="daterange" value-format="yyyy-MM-dd" range-separator="~" start-placeholder="完成时间" end-placeholder="完成时间"></el-date-picker>
        </div>
        <div>
        ...
        <div>
          <el-input size="mini" v-model="searchObj.jiedanzhanghao" placeholder="接单账号" clearable></el-input>
        </div>
         <div>
          <el-input size="mini" v-model="searchObj.agentId" placeholder="推广员ID" clearable></el-input>
        </div>
        <div>
          <el-input size="mini" v-model="searchObj.taobaoOrderId" placeholder="电商订单号" clearable></el-input>
        </div>
        <div>
        ...
        <div>
        <el-button size="mini" type="primary" icon="el-icon-download" @click="exportInformation()">导出</el-button>
      </div>
      </div>
    </div>
    ...
    <div class="orderManage-table">
      <el-table border ref="multipleTable" size="mini" :data="orderData" tooltip-effect="dark" style="width: 100%">
       ...
       <!--  <el-table-column fixed align="center" prop="status" label="订单状态" width="95">
          <template slot-scope="scope">
       ...
       <el-table-column align="center" prop="jiedanTime" label="接单时间"  width="200">
          ...
          <el-table-column align="center" prop="wanchengTime" label="完成时间"  width="200">
          <template slot-scope="scope">
            <span>{{scope.row.wanchengTime | dateParse}}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="scalpuser.id" label="买手ID" width="100"></el-table-column>
         <el-table-column align="center" prop="" label="接单账号" width="100"></el-table-column>
        <el-table-column align="center" label="推广员ID" width="80">
          <template slot-scope="scope">
            <span>{{scope.row.taskVM?(scope.row.taskVM.busines?(scope.row.taskVM.busines.promoter?scope.row.taskVM.busines.promoter.id:''):''):''}}</span>
          </template>
        </el-table-column>
  ...
  this.getOrderData(obj).then(r=>{
    ...
    loadAllOrder(){
       let obj={ 
        ...
        finishedBeginTime:this.searchObj.searchTime?config.passToStartDate(this.searchObj.searchTime[0]):'',
        finishedEndTime:this.searchObj.searchTime?config.passToEndDate(this.searchObj.searchTime[1]):'',
    ...
    toCancel(row){
     ...
     exportInformation() {
      this.$confirm('确认要导出么？', '提示', {
        ...
        taobaoOrderId:this.searchObj.taobaoOrderId,
        // status:this.searchObj.status,
        finishedBeginTime:this.searchObj.searchTime?this.searchObj.searchTime[0]:'',
        finishedEndTime:this.searchObj.searchTime?this.searchObj.searchTime[1]:'',
      };
      /* 删除空属性 */
      for(let key in obj){
        if(!obj[key]&&obj[key]!==0){
