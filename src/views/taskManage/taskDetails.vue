...
<template>
  ...
  <el-tabs type="card" style="overflow:hidden;" v-model="activeName"  @tab-click="handleClick">
      <el-tab-pane label="任务详情" name="taskDetails">任务详情</el-tab-pane>
      <el-tab-pane label="店铺详情" name="businessDetails">店铺详情</el-tab-pane>
...
export default {
  ...
  created(){
    ...
    // /* 通过id获取详细数据进行展示 */
    // service.get('/task/findTaskDetailsById', {params:{id:this.$route.query.id}}).then((data) => {
    //   if(data.status==200){
  ...
  methods: {
     handleClick(tab, event) {
      this.$router.push('/taskManage/taskDetails/'+tab.name);
    },
    toBack(){
